...
模拟图及其虚拟焦距/基线只验证流程。无匹配标定时可以看图和识别，但不输出物理距离。标定 RMS >2 px 显示红色/文字告警；较低 RMS 也不能代替卷尺实测误差。
...
实际通过的检查、实机性能统计口径和待验收事项见 [验证记录](VALIDATION.md)。
...
人工验收仍需完成：已知物距误差、相机实际拔插、锁屏恢复、横竖屏/截图、连续 30 分钟运行及温升。处理延迟统计从 UVC 帧到达 Rust 开始，不包含传感器曝光、USB 传输和最终屏幕显示延迟。
